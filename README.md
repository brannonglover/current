...
backend/             Next.js API + RSS ingestion
  ...
  data/phora.db    SQLite (local, gitignored)
...
| No `Detected Next.js` | Root Directory empty or wrong project linked | Confirm project **`current-backend`**, not Expo **`phora`** |
...
2. Link and deploy:
   ...
   Do **not** link at repo root to the **`phora`** Vercel project (Expo). Wrong link → `files should NOT have more than 15000 items`.
